...
class EditMUser extends StatefulWidget {
  final MUser muser;
  EditMUser({Key? key, required this.muser}) : super(key: key);

  @override
  State<EditMUser> createState() => _EditMUserState();
}

class _EditMUserState extends State<EditMUser> {
  ...
  Future<void> saveData() async {
    ...
    await _collectionRef.doc(widget.muser.id).update({
      'username': user,
      'password': pass,
    });
    if (!form!.validate()) {
  ...
  void initState() {
    ...
    super.initState();
    setState(() {
      userController.text = widget.muser.username!;
      passController.text = widget.muser.password!;
    });
  }
